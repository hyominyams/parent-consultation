CREATE OR REPLACE FUNCTION public.app_make_id()
RETURNS text
LANGUAGE sql
AS $$
  SELECT extensions.gen_random_uuid()::text
$$;
...
BEGIN
  ...
  IF FOUND THEN
    INSERT INTO "TeacherNotification" (
      ...
      v_reservation_id,
      '새 상담 신청',
      format('%s 학생이 %s 상담을 신청했습니다.', v_parent."studentName", to_char(v_slot."startDateTime", 'FMMM"월" FMDD"일" HH24:MI'))
    );
  END IF;
...
BEGIN
  ...
  FROM "Reservation"
  WHERE "parentUserId" = p_parent_user_id
  FOR UPDATE;
  ...
  IF FOUND THEN
    INSERT INTO "TeacherNotification" (
      ...
      v_teacher.id,
      CASE
        WHEN p_intent = 'reschedule' THEN '예약 변경 요청'
        ELSE '예약 취소'
      END,
      CASE
        WHEN p_intent = 'reschedule' THEN format('%s 학생이 기존 예약을 취소하고 새 시간을 선택하려고 합니다.', v_parent."studentName")
        ELSE format('%s 학생의 상담 예약이 취소되었습니다.', v_parent."studentName")
      END
    );
  END IF;
